...
<div class="container-fluid bg-light m-1 ">
  ...
  <div class="row m-2 bg-white shadow justify-content-center align-self-center" >
    <div class="col-12">
      <h5>Retention im Details vom <u>{{App\RetentionDetail::orderBy('call_date', 'asc')->limit(1)->value('call_date')->format('d.m.Y')}}</u> bis zum <u>{{App\RetentionDetail::orderBy('call_date', 'desc')->limit(1)->value('call_date')->format('d.m.Y')}}</u>  </h5>
      <h5>Daily Agent im Zeitraum vom <u>{{App\DailyAgent::orderBy('date', 'asc')->limit(1)->value('date')->format('d.m.Y H:i:s')}}</u>  bis zum <u>{{App\DailyAgent::orderBy('date', 'desc')->limit(1)->value('date')->format('d.m.Y H:i:s')}}</u> </h5>
      <h5>Stundenreport im Zeitraum vom <u>{{App\Hoursreport::orderBy('date', 'asc')->limit(1)->value('date')->format('d.m.Y H:i:s')}}</u>  bis zum <u>{{App\Hoursreport::orderBy('date', 'desc')->limit(1)->value('date')->format('d.m.Y H:i:s')}}</u> </h5>
    </div>
    <div class="col p-1">
      @php
